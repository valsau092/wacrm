...
export default function JoinPage() {
  ...
  const FAIL_COPY: Record<PeekFail['reason'], { title: string; body: string }> = {
    not_found: { title: t('fail.notFound.title'), body: t('fail.notFound.body') },
    used: { title: t('fail.used.title'), body: t('fail.used.body') },
    expired: { title: t('fail.expired.title'), body: t('fail.expired.body') },
    server_error: { title: t('fail.serverError.title'), body: t('fail.serverError.body') },
  };
  ...
  const inviteHeader = (
    <CardHeader className="items-center text-center">
      ...
      <CardTitle className="text-xl text-foreground">
        {t.rich('inviteTitle', {
          accountName: (chunks) => <span className="text-primary">{chunks}</span>,
