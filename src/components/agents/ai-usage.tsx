...
export function AiUsageCard() {
  ...
  const chartData =
    data?.daily.map((d) => ({ day: format(parseISO(d.date), 'MMM d', { locale: dateFnsLocale }), Tokens: d.tokens })) ??
    [];
      ...
      <CardContent className="space-y-5">
        {loading || !data ? (
          ...
          <div className="flex flex-col items-center justify-center gap-2 py-10 text-center text-sm text-muted-foreground">
            ...
            <p>{t('noUsageYet', { days: data.window_days })}</p>
            <p className="text-xs">
              {t('noUsageHint')}
            </p>
          </div>
                  ...
                  {data.by_model.map((m) => (
                    ...
                    >
                      ...
                      <span className="flex-shrink-0 tabular-nums text-muted-foreground">
                        {formatCompactNumber(m.tokens)} tok · {t('byModelCalls', { count: m.calls })}
                      </span>
                    </li>
                  ))}
                </ul>
