    ...
    try {
      ...
      if (!res.ok) {
        if (data.code === 'ai_not_configured') {
          ...
          toast.error(data.error ?? t('toastFailedReply'));
        }
    ...
    <div className="flex h-[60vh] min-h-[420px] flex-col rounded-xl border border-border bg-card">
      ...
      <div className="flex items-center justify-between border-b border-border px-4 py-3">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-medium text-foreground">{t('headerTitle')}</span>
        ...
        >
          <RotateCcw className="mr-1.5 h-3.5 w-3.5" /> {t('resetButton')}
        </Button>
      </div>
        ...
        {turns.length === 0 && (
          <div className="flex h-full flex-col items-center justify-center text-center text-sm text-muted-foreground">
            ...
            {onGoToSetup && (
              ...
              >
                {t('goToSetupLink')} <ArrowRight className="ml-1 h-3 w-3" />
              </Button>
            )}
          </div>
        )}
